...
describe('NgxSignInRedirectService', () => {
  ...
  describe('setRedirect', () => {
    ...
    it('should work with ActivatedRoute', () => {
      ...
      expect(setItemSpy).toHaveBeenCalledWith(config.storageKey, '/');
    });
    it('should work with ActivatedRouteSnapshot', () => {
      service.setRedirect(activatedRoute.snapshot);
      expect(setItemSpy).toHaveBeenCalled();
      expect(setItemSpy).toHaveBeenCalledWith(config.storageKey, '/');
    });
    it('should work with complex routes', () => {
      ...
      service.setRedirect(activatedRoute);
      expect(setItemSpy).toHaveBeenCalled();
      expect(setItemSpy).toHaveBeenCalledWith(config.storageKey, '/baz/bar/foo');
